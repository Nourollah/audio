...
def _parse_config(w2v_model):
    ...
    return {
        ...
        "encoder_attention_dropout": encoder.layers[
            0
        ].self_attn.dropout_module.p,
...
def _map_key(key):
    ...
    if re.match(r"(mask_emb|quantizer|project_q|final_proj|mask_emb)", key):
        return None
    if match := re.match(
        r"feature_extractor\.conv_layers\.0\.2\.(weight|bias)", key
    ):
        return f"feature_extractor.conv_layers.0.layer_norm.{match[1]}"
    if match := re.match(
        r"feature_extractor\.conv_layers\.(\d+)\.0\.(weight|bias)", key
    ):
        return f"feature_extractor.conv_layers.{match[1]}.conv.{match[2]}"
    if match := re.match(
        r"feature_extractor\.conv_layers\.(\d+)\.2\.1\.(weight|bias)", key
    ):
        return f"feature_extractor.conv_layers.{match[1]}.layer_norm.{match[2]}"
    if match := re.match(r"post_extract_proj\.(weight|bias)", key):
        return f"encoder.feature_projection.projection.{match[1]}"
    if match := re.match(r"layer_norm\.(weight|bias)", key):
        return f"encoder.feature_projection.layer_norm.{match[1]}"
    if match := re.match(
        r"encoder\.pos_conv\.0\.(bias|weight_g|weight_v)", key
    ):
        return f"encoder.transformer.pos_conv_embed.conv.{match[1]}"
    if match := re.match(r"encoder\.layer_norm\.(weight|bias)", key):
        return f"encoder.transformer.layer_norm.{match[1]}"
    if match := re.match(
        r"encoder\.layers\.(\d+)\.self_attn\.((k_|v_|q_|out_)proj\.(weight|bias))",
        key,
    ):
        return f"encoder.transformer.layers.{match[1]}.attention.{match[2]}"
    if match := re.match(
        r"encoder\.layers\.(\d+)\.self_attn_layer_norm\.(weight|bias)", key
    ):
        return f"encoder.transformer.layers.{match[1]}.layer_norm.{match[2]}"
    if match := re.match(r"encoder\.layers\.(\d+)\.fc1\.(weight|bias)", key):
        return f"encoder.transformer.layers.{match[1]}.feed_forward.intermediate_dense.{match[2]}"
    if match := re.match(r"encoder\.layers\.(\d+)\.fc2\.(weight|bias)", key):
        return f"encoder.transformer.layers.{match[1]}.feed_forward.output_dense.{match[2]}"
    if match := re.match(
        r"encoder\.layers\.(\d+)\.final_layer_norm\.(weight|bias)", key
    ):
        return f"encoder.transformer.layers.{match[1]}.final_layer_norm.{match[2]}"
    if match := re.match(r"proj\.(weight|bias)", key):
        return f"aux.{match[1]}"
    # HuBERT Extension
